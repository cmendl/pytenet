...
def test_mpo_orthonormalize():
    ...
    for backend in ["numpy", "torch"]:
        with ar.backend_like(backend):
            ...
            d = 4
            b = [1, 10, 13, 14, 7, 1]
            mpo0 = ptn.MPO(ptn.random_qnumbers(-2, 3, size=d, rng=rng),
            ...
            for i in range(mpo0.nsites):
                ...
                assert np.allclose(q.conj().T @ q, np.identity(s[3], like=q), rtol=1e-12), \
            ...
            for i in range(mpo0.nsites):
                s = mpo0.a[i].shape
                assert s[1] == d and s[2] == d
                q = np.transpose(mpo0.a[i], (3, 1, 2, 0)).reshape((s[1]*s[2]*s[3], s[0]))
...
def test_mpo_identity():
    ...
    for backend in ["numpy", "torch"]:
        with ar.backend_like(backend):
            ...
            qsite = ptn.random_qnumbers(-2, 3, size=d, rng=rng)
            ...
            assert np.allclose(idop.to_matrix(),
                               np.identity(d**nsites, like=idop.a[0]), rtol=0, atol=0), \
...
def test_mpo_from_opgraph():
    ...
    for backend in ["numpy", "torch"]:
        with ar.backend_like(backend):
            ...
            matref = graph.to_matrix(opmap)
            if backend == "torch" and torch.cuda.is_available():
                matref = ar.to_device(matref, "gpu")
            assert np.allclose(mpo.to_matrix(), matref)
...
def test_mpo_to_matrix():
    ...
    for backend in ["numpy", "torch"]:
        with ar.backend_like(backend):
            ...
            qsite = ptn.random_qnumbers(-2, 3, size=3, rng=rng)
            qbonds = [ptn.random_qnumbers(-2, 3, size=bi, rng=rng)
                      for bi in [1, 11, 16, 23, 19, 9, 1]]
            ...
            # compare
            assert np.allclose(mat_dense,
                               np.array(mat_sparse.toarray(), like=mat_dense),
                               rtol=1e-13), \
...
def test_mpo_multiply():
    ...
    for backend in ["numpy", "torch"]:
        with ar.backend_like(backend):
            ...
            # create random matrix product operators
            op0 = ptn.MPO(qsite, [ptn.random_qnumbers(-2, 3, size=bi, rng=rng)
                                  for bi in [1, 10, 13, 24, 17, 9, 1]],
                        fill="random", rng=rng)
            op1 = ptn.MPO(qsite, [ptn.random_qnumbers(-2, 3, size=bi, rng=rng)
