...
def test_block_sparse_qr():
    ...
    for backend in ["numpy", "torch"]:
        with ar.backend_like(backend):
            ...
            assert np.allclose(q.conj().T @ q,
                               np.identity(q.shape[1], like=q), rtol=1e-14), \
                "columns of `q` matrix must be orthonormalized"

            assert ptn.is_qsparse(q, [q0, ptn.neg_qnumbers(qinterm)]), \
                "sparsity pattern of `q` matrix must match quantum numbers"
            assert ptn.is_qsparse(r, [qinterm, ptn.neg_qnumbers(q1)]), \
...
def test_block_sparse_eigh():
    ...
    for backend in ["numpy", "torch"]:
        with ar.backend_like(backend):
            ...
            # enforce block sparsity structure dictated by quantum numbers
            ptn.enforce_qsparsity(a, [q0, ptn.neg_qnumbers(q0)])

            # perform diagonalization
            u, evals, qinterm = ptn.block_sparse_eigh(a, q0)

            assert np.allclose(u @ np.astype(np.diag(evals), u.dtype) @ u.conj().T,
                               a, rtol=1e-13), \
                "u @ evals @ u^H must match `a` matrix"

            assert np.allclose(u.conj().T @ u,
                               np.identity(u.shape[1], like=u), rtol=1e-14), \
    ...
    for backend in ["numpy", "torch"]:
        with ar.backend_like(backend):
            ...
            assert np.allclose(v @ v.conj().T,
                               np.identity(v.shape[0], like=v), rtol=1e-14), \
