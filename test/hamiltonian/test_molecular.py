                ...
                for opt in (True, False):
                    ...
                    for i in range(nsites + 1):
                        ...
                        if opt:
                            ...
                            b1 = 2 if 1 <= i <= nsites - 1 else 1
                        # a^{\dagger}_i and a_i chains,
                        # reaching (almost) from one boundary to the other
                        if opt:
                            b2 = 2 * min(nl**2 * (nl - 1) // 2, nr) \
                               + 2 * min(nl, nr**2 * (nr - 1) // 2)
                        else:
                            # slightly sub-optimal
        ...
        with ar.backend_like(backend):
            ...
            for nsites in range(4, 8):
                ...
                for i in range(nsites - 1):
                    ...
                    u2 = np.asarray(unitary_group.rvs(2, random_state=int(time.time())))

                    # extend to overall orbital rotation matrix
                    u = np.identity(nsites, like=u2)
                    u[i:i+2, i:i+2] = u2
                    ...
                    h_mpo_rotorb = ptn.molecular_hamiltonian_mpo(tkin_rotorb, vint_rotorb,
                                                                 optimize=False)
