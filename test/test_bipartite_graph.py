...
def test_hopcroft_karp():
    ...
    for backend in ["numpy", "torch"]:
        with ar.backend_like(backend):
            ...
            num_u = int(rng.integers(1, 101))
            num_v = int(rng.integers(1, 101))
            edges = []
            ...
            for (u, v) in matching:
                ulist.append(u)
                vlist.append(v)
            assert len(set(ulist)) == len(ulist)
            assert len(set(vlist)) == len(vlist)
            ...
            assert max_sms_len <= len(matching)
...
def test_minimum_vertex_cover():
    ...
    for backend in ["numpy", "torch"]:
        with ar.backend_like(backend):
            ...
            for (u, v) in edges:
                assert u in u_cover or v in v_cover
...
def stochastic_matching_search(graph: ptn.BipartiteGraph, rng):
    """
