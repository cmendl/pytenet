...
def lanczos_iteration(afunc, vstart, numiter):
    ...
    alpha = np.zeros(numiter,   dtype=float, like=vstart)
    beta  = np.zeros(numiter-1, dtype=float, like=vstart)

    v = np.zeros((numiter, len(vstart)), like=vstart)
    v[0] = vstart
    ...
    for j in range(numiter-1):
        ...
        if beta[j] < 100*len(vstart)*np.finfo(vstart.dtype).eps:
            warnings.warn(
...
def arnoldi_iteration(afunc, vstart, numiter):
    ...
    hess = np.zeros((numiter, numiter), like=vstart)
    v = np.zeros((numiter, len(vstart)), like=vstart)
    v[0] = vstart
    ...
    for j in range(numiter-1):
        ...
        if hess[j+1, j].real < 100*len(vstart)*np.finfo(vstart.dtype).eps:
            warnings.warn(
...
def eigh_krylov(afunc, vstart, numiter, numeig):
    ...
    u_ritz = v @ ar.astype(u_hess[:, 0:numeig], v.dtype)
...
def expm_krylov(afunc, vec, dt, numiter, hermitian=False):
    ...
    if hermitian:
        ...
        # diagonalize Hessenberg matrix
        w_hess, u_hess = eigh_tridiag(alpha, beta)
        z = np.linalg.norm(vec) * np.exp(dt*w_hess) * u_hess[0]
        return ar.astype(v, z.dtype) @ (ar.astype(u_hess, z.dtype) @ z)
    else:
        hess, v = arnoldi_iteration(afunc, vec, numiter)
        return v @ (np.linalg.norm(vec) * ar.do("linalg.expm", dt*hess)[:, 0])
